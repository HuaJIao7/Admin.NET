...
public class SysMenuService : IDynamicApiController, ITransient
{
    ...
    public async Task<List<SysMenu>> GetAllListID(long? id)
    {
        var entity = await _sysMenuRep.AsQueryable()
            .WhereIF(!id.IsNullOrEmpty(), u => u.Id == id)
    ...
    public async Task<List<SysMenu>> GetList([FromQuery] MenuInput input)
    {
        ...
        if (!string.IsNullOrWhiteSpace(input.Title) || input.Type is > 0)
        {
            return await query
                ...
                //.WhereIF(!string.IsNullOrWhiteSpace(name), u => u.Title.Contains(name))
