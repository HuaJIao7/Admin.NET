...
public class SysUserService : IDynamicApiController, ITransient
{
    ...
    {
        var entity = await _sysUserRep.AsQueryable()
            .WhereIF(!Org.IsNullOrEmpty(), u => u.OrgId == Org)
            .ClearFilter().ToListAsync();
        var entitys = await _sysUserRep.AsQueryable()
            .ClearFilter().ToListAsync();
        return entity;
    }
    ...
    {
        var entity = await _sysUserRep.GetByIdAsync(id);
            //.GetByIdAsync(_userManager.UserId);
        return entity;
    }
    ...
    {
        var enmtity = await _sysUserRoleService.GetUserRoleIdList(userId);
        var s = enmtity[0].ToString();
        var a = await GetBaseInfo(s);


        var entity = await _sysUserRep.GetByIdAsync(s);
        return entity;
